...
## Working with the NFTTickets contract

The NFTickets contract in the project is analogous a single event or conference. It is a demo contract and should not be considered "production ready", when building smart contracts to run on Mainnet Ethereum is a much more involved, detailed, and tested/auditied process. This is just enough of a contract that gives us the ability to build a frontend client that can read and write to the blockchain through minting NFTs.

The contract can be found in: `contracts/NFTTickets.sol`, it uses a `HexStrings.sol` file (a small smart contract `library`) as well it's migration file is #2

You will need to add a `.env` file with the following information:

```
INFURA_ENDPOINT_RINKEBY=wss://rinkeby.infura.io/ws/v3/[YOUR ID]
INFURA_ENDPOINT_POLYGON=wss://polygon-mumbai.infura.io/ws/v3/[YOUR ID]
OWNER_MNEMONIC="[YOUR MNEMONIC]"
...
We will update the `truffle-config` and need a package to help us out:

`yarn add @truffle/hdwallet-provider`

Then we can paste in the code required for dev, polygon and rinkeby testnets.
...
We have added a Solidity lib called `hexStrings.sol`:
```ts
...
As well our contract called `NFTickets.sol`:
```ts
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
contract NFTickets is ERC721Enumerable, Ownable {
  ...
  */
  ...
  function contractURI() public pure returns (string memory) {
    ...
    bytes memory collectionJsonString = bytes(abi.encodePacked(
      '{"name":"NFTickets",',
      '"description":"NFTickets is the most premier fox event in the solar system this year. Join friends from all over the world this December as the skulk descend on the Web3 space and dig their paws into everything Ethereum. Stay foxy, your NFT purchase is your ticket to the event!",', 
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
    ...
    string memory description = string(abi.encodePacked((vipTicketHolders[id] ? "VIP" : "General Admission"), ' access to NFTickets on Dec, 10, 2022.'));
...
Add a migration script named `2_setup_NFTickets`:
